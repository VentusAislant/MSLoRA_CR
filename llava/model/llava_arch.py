...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        cls_tokens, image_features = self.get_model().get_vision_tower()(images)

        image_features = self.get_model().mm_projector(image_features)

        # print(cls_tokens.shape, image_features.shape)
        # exit(0)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, position_ids, attention_mask, past_key_values, None, labels, getattr(self, 'cls_tokens', None)

        if type(images) is list or images.ndim == 5:
            concat_images = torch.cat([image for image in images], dim=0)
            image_features, cls_tokens = self.encode_images(concat_images)
            split_sizes = [image.shape[0] for image in images]
            image_features = torch.split(image_features, split_sizes, dim=0)
            image_features = [x.flatten(0, 1).to(self.device) for x in image_features]
        else:
            image_features, cls_tokens = self.encode_images(images)
            image_features, cls_tokens = image_features.to(self.device), cls_tokens.to(self.device)
        ...
        if _position_ids is None:
            position_ids = None
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, cls_tokens
